...
import Mascotte, { type MascotteHandle } from "@/components/Mascotte";
import { Smile } from "@/components/icons";
import { subscribeVel } from "@/lib/velBus";
import styles from "@/app/page.module.css";

function IntroMascotte() {
  ...
  return (
    ...
    >
      <Mascotte ref={mascotteRef} style={{ width: "100%", height: "auto", transform: "scaleX(-1) rotate(90deg)" }} />
    </div>
  );
}
...
function ProjectMascotte({
  ...
  transformMode?: "rotate90flip" | "rotate90" | "flipX" | "rotateNeg90";
  mascotteStyle: React.CSSProperties;
  ...
  useEffect(() => {
    ...
    const clearSpawnTimer = () => { if (spawnTimer) { clearTimeout(spawnTimer); spawnTimer = null; } };

    const onProgress = (e: Event) => {
      const { paletteA, t } = (e as CustomEvent).detail;
      const isActive = paletteA === paletteId;
      ...
      } else if (state.current === 0 && t >= 0.12) {
        clearSpawnTimer();
      }
    };
...
const shapeConfigs = [
  {
    ...
    triangle: { depthX: 0.18, depthY: 0.14, className: "absolute top-0 right-0",   style: { marginTop: "-25vh", marginRight: "2vw" },    mobileStyle: { marginTop: "0", top: "5vh", right: "2vw" } },
    arrow:    { depthX: 0.12, depthY: 0.09, className: "absolute bottom-0",         style: { right: "15vw", marginBottom: "-15vh" },      mobileStyle: { right: "5vw", marginBottom: "0", bottom: "5vh" } },
  },
  {
    ...
    arrow:    { depthX: 0.10, depthY: 0.08, className: "absolute bottom-0",         style: { left: "55vw", marginBottom: "-20vh" },       mobileStyle: { left: "60vw", marginBottom: "0", bottom: "5vh" } },
  },
  {
    circle:   { depthX: 0.08, depthY: 0.06, className: "absolute bottom-0 left-0", style: { marginBottom: "-10vh", marginLeft: "2vw" }, mobileStyle: { marginBottom: "0", bottom: "5vh", left: "2vw" } },
    triangle: { depthX: 0.16, depthY: 0.12, className: "absolute top-0 right-0",   style: { marginTop: "-20vh", marginRight: "18vw" },   mobileStyle: { marginTop: "0", top: "5vh", right: "2vw" } },
    arrow:    { depthX: 0.11, depthY: 0.09, className: "absolute top-0 left-0",    style: { marginTop: "-10vh", marginLeft: "2vw" }, flipY: true, mobileStyle: { marginTop: "0", top: "8vh", left: "2vw" } },
  },
];
...
export default function PortfolioClient({ projects }: { projects: Project[] }) {
  ...
  const spawnBall = () => { if (window.innerWidth >= 768) ballRef.current?.spawn(); };
  ...
  return (
    <>
      ...
      />
      <div ref={overlayRef} className="with-grain" style={{ position: "fixed", inset: 12, borderRadius: 12, background: "linear-gradient(225deg, #92FF33, #E2FF55)", zIndex: 999, display: "none", pointerEvents: "none" }} />
      <div data-white-circle style={{ position: "fixed", inset: 12, borderRadius: 12, backgroundColor: "white", backgroundImage: "radial-gradient(circle, rgba(0,0,0,0.18) 0.85px, transparent 0.85px)", backgroundSize: "22px 22px", zIndex: 2, display: "none", pointerEvents: "none" }} />
      <PhysicsBall ref={ballRef} />
      <HorizontalScroll loopEvery={loopEvery}>
        {COPIES.flatMap((copy) => [
          <div key={`intro-${copy}`} className={styles.introCard}>
            <svg width="74" height="82" viewBox="0 0 74 82" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ position: "absolute", bottom: "clamp(60px, 12vh, 160px)", left: "clamp(16px, 3vw, 48px)", width: "clamp(36px, 7vw, 80px)", height: "auto", pointerEvents: "none", zIndex: 2 }}>
            ...
            <div className={styles.introGroup}>
              ...
              <div style={{ position: "relative", display: "inline-block" }}>
                ...
                <svg width="166" height="160" viewBox="0 0 166 160" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ position: "absolute", bottom: "clamp(-50px, -4vw, -20px)", right: "clamp(-25px, -3vw, -10px)", width: "clamp(48px, 11vw, 120px)", height: "auto", pointerEvents: "none" }}>
