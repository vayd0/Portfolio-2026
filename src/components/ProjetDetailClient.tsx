...
export default function ProjetDetailClient({ project, palette }: Props) {
  ...
  useEffect(() => {
    ...
    gsap.set(el, { clipPath: "circle(0vmax at 50% 50%)" });
  ...
  const LinksSection = () => (
    <div ref={linksRef} style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center", position: "relative" }}>
      ...
      {project.projectUrl && <AnnotationEl />}
    </div>
  );
  ...
  return (
    ...
    >
      ...
      {isMobile ? (
        <div style={{ position: "relative", zIndex: 2, padding: "clamp(20px, 6vw, 48px)", display: "flex", flexDirection: "column", gap: 24 }}>
          <RetourBtn />
          <div
            ref={titleRef}
            style={{
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              {project.gallery.slice(0, 3).map((src, gi) => (
                <div
                  ...
                  style={{ width: "100%", aspectRatio: "16/9", borderRadius: 10, overflow: "hidden", boxShadow: "0 6px 24px rgba(0,0,0,0.10)" }}
                >
        ...
        <div
          style={{
            ...
            padding: "clamp(20px, 3.5vw, 52px) clamp(24px, 4vw, 64px)",
          }}
        >
          <RetourBtn />
          <div
              ...
              <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "clamp(10px, 1.5vh, 20px)", minHeight: 0 }}>
                {project.gallery.slice(0, 2).map((src, gi) => (
                  <div
                    key={gi}
                    ref={(el) => { galleryRefs.current[gi] = el; }}
                    style={{ flex: 1, minHeight: 0, borderRadius: 10, overflow: "hidden", boxShadow: "0 6px 24px rgba(0,0,0,0.10)" }}
                  >
                    <img src={imgSrc(src)} alt={`${project.title} ${gi + 1}`} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
                  </div>
                ))}
