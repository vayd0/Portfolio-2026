...
function ContactButton() {
  ...
  useEffect(() => {
    ...
    const stopEls = Array.from({ length: 4 }, () => {
      const el = document.createElementNS(ns, "stop");
      grad.appendChild(el);
      return el;
    });

    const apply = (pA: string, pB: string, t: number) => {
      ...
      const [c1A, , c2A] = PALETTE_COLORS[pA] ?? ["#fff", "#fff", "#fff"];
      const [c1B, , c2B] = PALETTE_COLORS[pB] ?? ["#fff", "#fff", "#fff"];

      const boundary = (1 - t) * window.innerWidth;
      const splitOL = Math.max(0, Math.min(W, boundary - btnRect.left));

      let stopData: Array<[number, string]>;
      if (splitOL <= 0) {
        stopData = [[0, c1B], [1, c2B], [1, c2B], [1, c2B]];
      } else if (splitOL >= W) {
        stopData = [[0, c1A], [1, c2A], [1, c2A], [1, c2A]];
      } else {
        const f = splitOL / W;
        stopData = [[0, c1A], [f, c2A], [f, c1B], [1, c2B]];
      }

      stopData.forEach(([offset, color], i) => {
        stopEls[i].setAttribute("offset", `${(offset * 100).toFixed(1)}%`);
        stopEls[i].setAttribute("stop-color", color);
      });

      const clipA = splitOL <= 0 ? "inset(0 200% 0 0)" : splitOL >= W ? "" : `inset(0 ${(W - splitOL).toFixed(1)}px 0 0)`;
      const clipB = splitOL <= 0 ? "" : splitOL >= W ? "inset(0 200% 0 0)" : `inset(0 0 0 ${splitOL.toFixed(1)}px)`;
      if (spanARef.current) { spanARef.current.style.color = c1A; spanARef.current.style.clipPath = clipA; }
      if (spanBRef.current) { spanBRef.current.style.color = c1B; spanBRef.current.style.clipPath = clipB; }
    };

    const [c1Init, , c2Init] = PALETTE_COLORS["0"];
    const initData: Array<[number, string]> = [[0, c1Init], [1, c2Init], [1, c2Init], [1, c2Init]];
    initData.forEach(([offset, color], i) => {
      stopEls[i].setAttribute("offset", `${(offset * 100).toFixed(1)}%`);
      stopEls[i].setAttribute("stop-color", color);
    });
    if (spanARef.current) { spanARef.current.style.color = c1Init; spanARef.current.style.clipPath = ""; }
    if (spanBRef.current) { spanBRef.current.style.color = c1Init; spanBRef.current.style.clipPath = "inset(0 200% 0 0)"; }

    apply("0", "0", 0);

    const handler = (e: Event) => {
      const { paletteA, paletteB, t } = (e as CustomEvent<{ paletteA: string; paletteB: string; t: number }>).detail;
      apply(paletteA || "0", paletteB || "0", t);
    };

    window.addEventListener("section:scroll-progress", handler);
    return () => {
      window.removeEventListener("section:scroll-progress", handler);
      stopEls.forEach(s => s.remove());
    };
  }, []);
  ...
  return (
    <a
      ...
      onMouseLeave={onLeave}
    >
      <svg width="296" height="104" viewBox="0 0 296 104" fill="none" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <linearGradient ref={gradRef} id="btnBorderGrad" gradientUnits="userSpaceOnUse" x1="0" y1="52" x2="296" y2="52" />
        </defs>
        <rect x="16.0107" y="12.88" width="267" height="78" fill="black"/>
        <line x1="15.207" x2="15.207" y2="104" stroke="url(#btnBorderGrad)" strokeWidth="4.5"/>
        <line x1="283.207" x2="283.207" y2="104" stroke="url(#btnBorderGrad)" strokeWidth="4.5"/>
        <line x1="-2.24183e-07" y1="91.3499" x2="296" y2="91.3498" stroke="url(#btnBorderGrad)" strokeWidth="4.5"/>
        <line x1="2.24183e-07" y1="12.3499" x2="296" y2="12.3499" stroke="url(#btnBorderGrad)" strokeWidth="4.5"/>
      </svg>
      {[{ ref: spanARef, clipId: "nav_mail_clip_a" }, { ref: spanBRef, clipId: "nav_mail_clip_b" }].map(({ ref, clipId }) => (
        <span
          key={clipId}
          ref={ref}
          style={{
            ...
            color: PALETTE_COLORS["0"][0],
            marginTop: "-4px",
          }}
        >
          <svg width="30" height="30" viewBox="0 0 59 59" style={{ marginTop: 7 }} fill="none" xmlns="http://www.w3.org/2000/svg">
